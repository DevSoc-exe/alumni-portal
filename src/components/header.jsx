...
export default function Header({user_firstName}) {
    const [dropdownState, setDropdownState] = useState(false);

    return (
        <header className='w-11/12 flex justify-between m-6 mt-10'>
            <span className='text-darkBlue text-2xl font-extrabold'>Alumni Portal</span>
            <div className='flex relative justify-between items-center gap-2'>
                ...
                <span className='max-w-[150px] font-bold truncate'>{user_firstName}</span>
                <Dropdown 
                />
            </div>
